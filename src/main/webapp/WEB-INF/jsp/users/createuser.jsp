...
<head>
    ...
    <title>Parkiyo | Create User</title>
    ...
    <link
            ...
            rel="stylesheet" />
    <script>tailwind.config = { darkMode: "class", theme: { extend: { colors: { primary: "#1f68f9", "background-dark": "#020617" }, fontFamily: { display: ["Public Sans", "sans-serif"] }, borderRadius: { squircle: "14px" } } } }</script>
    <style>
        body {
        ...
        <header
            ...
            <div class="flex items-center gap-4">
                <button onclick="window.location.href='usermanagement.html'"
                        class="h-10 w-10 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center hover:bg-white/10 transition-all"><span
                        class="material-symbols-outlined text-slate-400">arrow_back</span></button>
                <div>
                    <h2 class="text-xl font-black text-white">Create New User</h2>
                    <p class="text-[11px] uppercase tracking-[0.18em] text-slate-500 font-black mt-1">Admin-Only
                        Account Creation</p>
                </div>
            </div>
            <div class="flex items-center gap-4">
            ...
            <div class="max-w-2xl mx-auto">
                <div class="glass-card rounded-[2.5rem] p-10">
                    <div class="flex items-center gap-4 mb-8">
                        <div
                                class="h-14 w-14 rounded-2xl bg-primary/10 border border-primary/20 flex items-center justify-center">
                            <span class="material-symbols-outlined text-primary text-2xl">person_add</span>
                        </div>
                        <div>
                            <h3 class="text-xl font-black text-white">New Account</h3>
                            <p class="text-slate-400 text-sm font-medium">Admins can create accounts with any role
                                including Admin.</p>
                        </div>
                    </div>

                    <div id="formMsg"
                         class="hidden mb-6 px-4 py-4 rounded-2xl text-[13px] font-bold flex items-center gap-3">
                        <span id="msgIcon" class="material-symbols-outlined text-base shrink-0">info</span>
                        <p id="msgText"></p>
                    </div>

                    <form id="createForm" class="space-y-5" novalidate>
                        <div class="grid grid-cols-2 gap-5">
                            <div><label class="block text-sm font-bold text-slate-200 ml-1 mb-2">First
                                Name</label><input type="text" id="fn"
                                                   class="w-full h-14 rounded-2xl px-5 input-glass font-medium"
                                                   placeholder="First name" /></div>
                            <div><label class="block text-sm font-bold text-slate-200 ml-1 mb-2">Last
                                Name</label><input type="text" id="ln"
                                                   class="w-full h-14 rounded-2xl px-5 input-glass font-medium"
                                                   placeholder="Last name" /></div>
                        </div>
                        <div><label class="block text-sm font-bold text-slate-200 ml-1 mb-2">Email
                            Address</label><input type="email" id="email"
                                                  class="w-full h-14 rounded-2xl px-5 input-glass font-medium"
                                                  placeholder="user@parkiyo.com" /></div>
                        <div><label class="block text-sm font-bold text-slate-200 ml-1 mb-2">Phone
                        ...
                        <div class="grid grid-cols-2 gap-5">
                            <div>
                                ...
                                <select id="role"
                                        class="w-full h-14 rounded-2xl px-5 input-glass font-medium appearance-none cursor-pointer">
                        ...
                        <div><label class="block text-sm font-bold text-slate-200 ml-1 mb-2">Password</label><input
                                type="password" id="pw" class="w-full h-14 rounded-2xl px-5 input-glass font-medium"
                                placeholder="Set a password" /></div>
                        <div><label class="block text-sm font-bold text-slate-200 ml-1 mb-2">Confirm
                            Password</label><input type="password" id="cpw"
                                                   class="w-full h-14 rounded-2xl px-5 input-glass font-medium"
                                                   placeholder="Confirm password" /></div>
                        <div class="flex gap-4 pt-2">
                            <button type="button" onclick="resetForm()"
                                    class="flex-1 h-14 bg-white/5 border border-white/10 text-slate-300 font-bold rounded-2xl hover:bg-white/10 transition-all">Reset
                                All</button>
                            <button type="submit"
                                    class="flex-1 h-14 bg-primary text-white font-black rounded-2xl shadow-lg shadow-primary/20 hover:scale-[1.02] transition-all flex items-center justify-center gap-2">Create
                                Account <span class="material-symbols-outlined">person_add</span></button>
                        </div>
                    </form>
                </div>
            </div>
...
<script>
    ...
    document.getElementById('createForm').addEventListener('submit', e => {
        ...
        if (!fn || !ln) { showMsg('error', 'Enter first and last name.'); return; }
        if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) { showMsg('error', 'Enter a valid email address.'); return; }
        if (!phone) { showMsg('error', 'Enter a phone number.'); return; }
        if (pw.length < 8) { showMsg('error', 'Password must be at least 8 characters.'); return; }
        if (pw !== cpw) { showMsg('error', 'Passwords do not match.'); return; }
        if (usedEmails.includes(email)) { showMsg('error', 'This email is already in use.'); return; }
        showMsg('success', `Account created for ${fn} ${ln}. Redirecting...`);
